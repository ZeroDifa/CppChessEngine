...
public:
    ...
    double status() {
        ...
        for (const auto& entry : piecePositions) {
            ...
            double sign = pieceColor == WHITE ? 1. : -1.;
            for (int i : entry.second) {
                int mobility = getAllowCages(i, false).size();
                status += DEFAULT_COST.at(pieceType)*sign + SQUARE_COST.at(entry.first)[i] + mobility/10*sign;
            }
        }
